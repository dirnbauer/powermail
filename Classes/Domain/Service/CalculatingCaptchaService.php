...
class CalculatingCaptchaService
{
    ...
    protected function getTextPositionForCaptcha(
        ...
        $minX = min(
            $this->getBoundingBoxCoordinate($boundingBox, 0),
            $this->getBoundingBoxCoordinate($boundingBox, 2),
            $this->getBoundingBoxCoordinate($boundingBox, 4),
            $this->getBoundingBoxCoordinate($boundingBox, 6)
        );
        $maxX = max(
            $this->getBoundingBoxCoordinate($boundingBox, 0),
            $this->getBoundingBoxCoordinate($boundingBox, 2),
        ...
        $minY = min(
            $this->getBoundingBoxCoordinate($boundingBox, 1),
            $this->getBoundingBoxCoordinate($boundingBox, 3),
            $this->getBoundingBoxCoordinate($boundingBox, 5),
            $this->getBoundingBoxCoordinate($boundingBox, 7)
        );
        $maxY = max(
            $this->getBoundingBoxCoordinate($boundingBox, 1),
            $this->getBoundingBoxCoordinate($boundingBox, 3),
    ...
    /**
     * @param array<mixed> $boundingBox
     * @throws FileCannotBeCreatedException
     */
    protected function getBoundingBoxCoordinate(array $boundingBox, int $index): int
    {
        $coordinate = $boundingBox[$index] ?? null;
        if (!is_int($coordinate)) {
            throw new FileCannotBeCreatedException(
                'Captcha text bounding box contains an invalid coordinate',
                1755500003
            );
        }
